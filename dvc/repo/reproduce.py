...
def _start_ready_stages(
    ...
    *,
    max_stages: int,
    mutex_groups: defaultdict[str, set["Stage"]],
        ...
        stage = next(
            (
                ...
                if (
                    ...
                    and all(len(mutex_groups[mutex]) == 0 for mutex in stage.mutex)
                )
            ),
            None,
        )
...
def _remove_mutexes(
    ...
    for stage in stages:
        for mutex in stage.mutex:
            group_stages = mutex_groups[mutex]
...
def _reproduce(
    ...
    **kwargs,
    ...
    mutex_groups: defaultdict[str, set[Stage]] = defaultdict(set)
    to_repro = {
        stage: StageInfo(
